...
# #     _supported_formats = {'.pdf', '.txt', '.html', '.docx', '.doc'}  # Class variable

# #     # def __init__(self):
# #         # Define supported formats and their handlers
# #         # self._supported_formats = {
# #         #     '.pdf': self._read_pdf,
# #         #     '.txt': self._read_text,
# #         #     '.html': self._read_html,
# #         #     '.docx': None,  # Placeholder for future implementation
# #         #     '.doc': None    # Placeholder for future implementation
# #         # }

# #     def __init__(self):
# #         self.supported_formats = self._supported_formats.copy()  # Instance variable
                
# #     # @property
# #     # def supported_formats(self) -> set:
# #     #     """Get set of supported file formats"""
# #     #     return set(self._supported_formats.keys())
...
# #         max_chunk_size: int = 60000  # Safe limit for Milvus
# #     ):
...
# #         self.sliding_splitter = CharacterTextSplitter(
# #             chunk_size=min(chunk_size, max_chunk_size),
# #             chunk_overlap=chunk_overlap,
# #             separator=" "
# #         )
...
# #                 sub_chunks = self._split_large_chunk(chunk.page_content)
...
# #             for hit in hits:
# #                 doc = self.collection.query(expr=f"id == {hit.id}")[0]
# #                 results.append({
# #                     "id": hit.id,
# #                     "text": doc["text"],
# #                     "score": hit.score,
...
# #                 "metric_type": "COSINE",
# #                 "index_type": "IVF_FLAT",
# #                 "params": {"nlist": 128}  # Reduced for smaller datasets
# #             }
...
# #             # Store full texts in metadata if truncated
...
# #                     self.collection.flush()
# #                 except Exception as e:
# #                     logger.error(f"Failed to insert batch {i}-{end_idx}: {str(e)}")
# #                     continue
...
# #                         # Get text (either from document or metadata)
# #                         text = doc.get("text", "")
...
# #         chunks = self._split_text_for_summary(text)
# #         summaries = []
...
#             selected_sentences = sentences[:3]  # Select top 3 sentences
...
#             Aşağıdakı məlumatlar əsasında qısa və dəqiq cavab hazırlayın:
...
# #         prompt_template = {
# #             "az": """Mətn əsasında 5 sual yaradın. Suallar mətnin əsas mövzularını əhatə etməlidir.
            
# #             Mətn:
# #             {text}
            
# #             Suallar:""",
# #             "en": """Generate 5 questions based on the text. Questions should cover the main topics of the text.
...
# #         prompt = prompt_template[language].format(text=text[:3000])  # Limit text length
...
# #             print("\nGenerated questions:")
# #             for idx, question in enumerate(doc_result['questions'], 1):
# #                 print(f"{idx}. {question}")
            
# #             print("\nExtractive summary:")
# #             print(doc_result['extractive_summary'])
            
# #             print("\nAbstractive summary:")
# #             # print(doc_result['abstractive_summary'])
            
# #             # Example query
# #             query = "Əmək haqqı kartlarının sifarişi prosesi necə həyata keçirilir?"
# #             # query_result = pipeline.query(query, top_k=5)
# #             query_result = pipeline.query(query, use_hyde=True, top_k=5)
...
# #         else:
# #             print(f"Error: {doc_result['message']}")
...
#         # Add logging to see chunk details
#         logging.getLogger('ChunkingStrategy').setLevel(logging.DEBUG)        
...
#             # List of all queries (including the new ones)
#             queries = [
#                 "Əmək haqqı kartlarının sifarişi prosesi necə həyata keçirilir?",
#                 "Karyera inkişafı üzrə təklif kim tərəfindən təqdim edilir?",
#                 "Sifariş edilmiş kartlar 90 gün ərzində təhvil alınmadıqda məhv edilir? Konkret olaraq bir sözlə (bəli və ya xeyr) cavab ver.",
#                 "Kiçik mütəxəssislərin vəzifə artımı üçün hansı meyarlar nəzərə alınır?",
#                 "BDK nədir?"
#             ]
            
#             # Loop through each query and get results
...
#                 if query_result['status'] == 'success':
#                     print("\nQuery Variations:")
#                     for var in query_result['query_variations']:
#                         print(f"- {var}")
...
#                         print(f"\nRank: {result['rank']}")
#                         print(f"Score: {result['score']:.4f}")
#                         print(f"Matching Query: {result['matching_query']}")
#                         print(f"Text: {result['text']}")
#                 else:
...
#         try:
#             # Drop existing collection if it's too large
#             utility.drop_collection(self.collection_name)
#             logger.info(f"Dropped collection {self.collection_name} to free space")
#         except Exception as e:
...
# # class MilvusVectorStore:
#     """Complete Milvus vector store implementation with enhanced features and error handling"""
...
#         port: int = 19530,
#         embedding_dim: int = 1024,
#         max_text_length: int = 32766,  # Milvus varchar limit
#         max_retries: int = 3,
#         retry_delay: int = 2,
#         max_collection_size: int = 1000000,  # Maximum number of vectors
#         batch_size: int = 32  # Default batch size for operations
#     ):
#         """Initialize MilvusVectorStore with configuration parameters"""
#         self.collection_name = collection_name
#         self.embedding_dim = embedding_dim
#         self.max_text_length = max_text_length
#         self.max_retries = max_retries
...
#         self._initialize_with_cleanup()

#     def _initialize_with_cleanup(self):
#         """Initialize connection and ensure clean state"""
#         try:
...
#             connections.disconnect("default")
#         except Exception as e:
#             logger.debug(f"Disconnect notice: {str(e)}")

#     def _cleanup_existing_collections(self):
#         """Comprehensive cleanup of existing collections"""
#         try:
#             existing_collections = utility.list_collections()
...
#                 try:
#                     # Load collection to check stats
...
#                         logger.info(f"Collection statistics - Row count: {row_count}")
                        
#                         if row_count > self.max_collection_size:
#                             logger.warning(f"Collection size ({row_count}) exceeds maximum ({self.max_collection_size})")
#                             self._force_cleanup_collection()
                        
#                     except Exception as stats_error:
#                         logger.error(f"Failed to get collection statistics: {str(stats_error)}")
#                         self._force_cleanup_collection()
...
#     def _force_cleanup_collection(self):
#         """Force cleanup of collection with multiple attempts"""
#         max_cleanup_attempts = 3
        
#         for attempt in range(max_cleanup_attempts):
#             try:
#                 logger.info(f"Attempting to force cleanup collection (attempt {attempt + 1})")
                
#                 # First try to release
#                 try:
#                     collection = Collection(self.collection_name)
#                     collection.release()
#                 except:
...
#                     raise RuntimeError(f"Failed to cleanup collection after {max_cleanup_attempts} attempts: {str(e)}")
...
#                 "metric_type": "COSINE",
#                 "index_type": "IVF_FLAT",
#                 "params": {"nlist": min(128, self.max_collection_size // 1000)}
#             }
...
#     def _truncate_text(self, text: str) -> str:
#         """Safely truncate text to fit Milvus varchar limit"""
#         if len(text) > self.max_text_length:
#             return text[:self.max_text_length-3] + "..."
#         return text
...
#         batch_size: int = None
#     ):
#         """Insert documents with enhanced error handling and batching"""
...
#             # Insert in batches with retries
#             total_batches = (len(processed_texts) + batch_size - 1) // batch_size
...
#                 self._insert_batch(batch_data, batch_num, total_batches)
...
#     def _insert_batch(self, batch_data, batch_num, total_batches):
#         """Insert a single batch with retries"""
#         for attempt in range(self.max_retries):
#             try:
#                 logger.info(f"Inserting batch {batch_num}/{total_batches}")
                
#                 # Check collection size before insert
#                 try:
#                     stats = self.collection.get_statistics()
#                     current_size = int(stats.get('row_count', 0))
                    
#                     if current_size + len(batch_data[0]) > self.max_collection_size:
#                         logger.warning("Collection size limit would be exceeded. Performing cleanup...")
#                         self._force_cleanup_collection()
#                         self._create_collection()
#                 except:
#                     pass
...
#             except Exception as e:
#                 if attempt == self.max_retries - 1:
#                     raise RuntimeError(f"Failed to insert batch {batch_num} after {self.max_retries} attempts: {str(e)}")
#                 logger.warning(f"Batch {batch_num} insert failed (attempt {attempt + 1}), retrying...")
#                 time.sleep(self.retry_delay)
#                 self._handle_insert_error()

#     def _handle_insert_error(self):
#         """Handle insert errors with recovery attempts"""
#         try:
...
#         limit: int = 5
#     ) -> List[Dict]:
#         """Perform hybrid search with error handling and retries"""
#         for attempt in range(self.max_retries):
#             try:
#                 logger.info("Starting hybrid search")
...
#                 search_params = {
#                     "metric_type": "COSINE",
#                     "params": {"nprobe": min(10, self.max_collection_size // 1000)}
#                 }
...
#                 logger.info(f"Found {len(search_results[0])} results")
...
#         except Exception as e:
#             logger.debug(f"Cleanup notice: {str(e)}")
...
# class ResultRepacker:
#     """Repackages and reorganizes search results"""
...
#         reverse: bool = True
#     ) -> List[Dict]:
#         """Repack results with optional reversal"""
#         try:
#             # Sort by score
#             sorted_results = sorted(
#                 results,
#                 key=lambda x: float(x.get('rerank_score', 0.0)),
#                 reverse=not reverse  # Reverse if requested
#             )
            
#             # Repack with new ranks
#             repacked_results = []
...
#         except Exception as e:
#             logger.error(f"Result repacking failed: {str(e)}")
#             return results
...
# class DocumentSummarizer:
#     """Handles summarization of search results for LLM context"""
...
#     ):
#         """Initialize the summarizer with configurable parameters"""
#         self.max_length = max_length
...
#         language: str = "az"
#     ) -> str:
#         """Create a focused summary of search results for LLM context"""
#         try:
#             if not results:
#                 return self._get_default_message(language)
            
#             # Prepare context with query focus
#             context = self._prepare_context(results, query, language)
            
#             # Generate initial summary
#             summary = self._generate_summary(context)
            
#             # Enhance summary with key information
#             enhanced_summary = self._enhance_summary(summary, results, query, language)
...
#         language: str
#     ) -> str:
#         """Prepare context for summarization with proper focus"""
        
#         prompt_template = {
#             "az": f"""Sual: {query}

#             Aşağıdakı məlumatlar əsasında cavab hazırlayın.
#             Məlumatlar əhəmiyyət dərəcəsinə görə sıralanıb:

#             """,
#             "en": f"""Question: {query}

#             Prepare an answer based on the following information.
#             Information is ranked by importance:

#             """
#         }[language]
        
#         # Add ranked results with scores
#         context = prompt_template
#         total_score = sum(float(r.get('score', 0.0)) for r in results)
...
#             importance = score / total_score if total_score > 0 else 0
...
#         """Generate initial summary with chunking for long contexts"""
#         try:
#             # Split long context if needed
...
#                 return " ".join(summaries)
#             else:
#                 # Summarize whole context if it's short enough
...
#             return context[:self.max_length]  # Fallback to truncation
...
#         language: str
#     ) -> str:
#         """Enhance summary with key information and structure"""
#         try:
...
#     def _extract_key_info(self, results: List[Dict]) -> str:
#         """Extract key information from results"""
#         key_points = []
#         seen_points = set()
...
#                 # Only add informative, non-duplicate sentences
#                 if (len(sentence) > 20 and  # Avoid very short sentences
#                     sentence not in seen_points and
#                     not any(self._is_similar(sentence, point) for point in seen_points)):
...
#         """Check if two pieces of text are similar using basic comparison"""
...
#     ) -> List[str]:
#         """Split long context into smaller chunks for processing"""
#         chunks = []
#         current_chunk = []
#         current_size = 0
...
#         """Get default message when summarization fails"""
#         return {
#             "az": "Bu sual üçün kifayət qədər məlumat tapılmadı.",
#             "en": "Insufficient information found for this query."
#         }[language]

#     def __del__(self):
#         """Cleanup when summarizer is destroyed"""
#         try:
...
# # new working ragpipeline and main func for reranking, repacking, llm_resp ..
# class RAGPipeline:
#     def __init__(
#         self,
#         collection_name: str = "azerbaijan_docs",
...
#         embedding_model: str = "BAAI/bge-m3",
#         llm_model: str = "unsloth/Llama-3.2-1B-Instruct",
#         host: str = "localhost",
#         port: int = 19530,
#         language: str = "az",
#         device: str = None,
...
#             # Initialize components in pipeline order
#             logger.info("Initializing pipeline components...")
...
#             # 3. Reranking and Repacking
#             self.reranker = ReRanker()
#             self.repacker = ResultRepacker()

#             # 4. Summarization and Response Generation
#             self.summarizer = DocumentSummarizer()
...
#             # Initialize Milvus connection last
#             logger.info("Initializing Milvus connection...")
#             self.vector_store = None  # Initialize as None first
#             self._init_vector_store(collection_name, host, port)
...
#     def _init_vector_store(self, collection_name: str, host: str, port: int):
#         import time
...
#                     logger.warning(f"Vector store initialization attempt {attempt + 1} failed, retrying in {retry_delay}s...")
#                     time.sleep(retry_delay)
#                 else:
#                     raise RuntimeError(f"Failed to initialize vector store after {max_retries} attempts")


#     def _check_vector_store(self):
#         """Check vector store connection and reinitialize if needed"""
...
#         except Exception:
#             logger.warning("Vector store connection lost, attempting to reconnect...")
#             self._init_vector_store(
#                 self.vector_store.collection_name,
#                 self.vector_store.host,
#                 self.vector_store.port
#             )
...
#         context: str
#     ) -> str:
#         """Generate LLM response using summarized context"""
#         try:
#             prompt_template = {
#                 "az": f"""Sual: {query}

#                 Verilmiş məlumatlar əsasında dəqiq və ətraflı cavab verin:
...
#                 Based on the given information, provide a precise and detailed answer:
...
#             return "Cavab yaratmaq mümkün olmadı."
...
#         expand_queries: bool = True
#     ) -> Dict[str, Any]:
#         """Process a query through the correct pipeline flow"""
#         try:

#             # Check vector store connection
#             self._check_vector_store()

#             # 1. Query Classification and Augmentation
#             if expand_queries:
#                 logger.info("Generating query variations")
#                 all_queries = self.question_generator.generate_questions(
#                     query,
...
#             # 4. Repacking Phase
#             logger.info("Repacking results")
#             repacked_results = self.repacker.repack_results(reranked_results, query)

#             # 5. Summarization Phase
#             logger.info("Generating context summary")
#             context_summary = self.summarizer.summarize_results(
#                 repacked_results,
#                 query,
#                 language=self.language
#             )
...
#             llm_response = self.generate_llm_response(query, context_summary)

#             # Format final results
#             formatted_results = []
#             for idx, result in enumerate(repacked_results[:top_k], 1):
#                 formatted_results.append({
#                     'rank': idx,
#                     'text': clean_text(result.get('text', '')),
#                     'score': float(result.get('score', 0.0)),
...
# def main():
#     """Example usage of the updated RAG pipeline"""
#     import psutil
#     import time
#     from datetime import datetime
...
#         print(f"\nCurrent memory usage: {memory:.2f} MB")
...
#             print(f"- Total chunks: {doc_result['chunks']}")
#             print(f"- Character count: {doc_result['metadata'].get('char_count', 'N/A')}")
#             print(f"- Word count: {doc_result['metadata'].get('word_count', 'N/A')}")
#             print_memory_usage()
...
#                 "Əmək haqqı kartlarının sifarişi prosesi necə həyata keçirilir?",
#                 "Karyera inkişafı üzrə təklif kim tərəfindən təqdim edilir?",
#                 "Sifariş edilmiş kartlar 90 gün ərzində təhvil alınmadıqda məhv edilir?",
#                 "Kiçik mütəxəssislərin vəzifə artımı üçün hansı meyarlar nəzərə alınır?",
#                 "BDK nədir?"
#             ]
...
#                     print("\n1. Query Variations:")
#                     for var in result['query_variations']:
#                         print(f"- {var}")
                    
#                     print("\n2. Retrieved and Ranked Results:")
#                     for r in result['results']:
#                         print(f"\n[Rank {r['rank']} | Score: {r['score']:.4f}]")
#                         print(f"Matching Query: {r['matching_query']}")
#                         print(f"Text: {r['text']}")
...
#         else:
#             print(f"Error processing document: {doc_result['message']}")
...
#     """Manages Milvus storage and cleanup"""
...
#         docker_volume: str = "milvus-standalone-docker-compose_milvus-etcd-data"
#     ):
#         self.host = host
#         self.port = port
...
#         """Check available storage space in Milvus volume"""
#         try:
...
#             # Parse output to find volume usage
#             lines = result.stdout.split('\n')
#             for line in lines:
#                 if self.docker_volume in line:
#                     parts = line.split()
#                     total = float(parts[1].replace('GB', ''))
#                     used = float(parts[2].replace('GB', ''))
#                     return total - used, total
                    
#             return 0, 0
            
#         except Exception as e:
#             self.logger.error(f"Failed to check storage space: {str(e)}")
#             return 0, 0
...
#             # Sort collections by creation time (if available in metadata)
#             # For now, we'll just remove the first collection in the list
#             if len(collections) > 0:
#                 oldest_collection = collections[0]
#                 utility.drop_collection(oldest_collection)
#                 self.logger.info(f"Dropped collection: {oldest_collection}")
...
#             # Remove volumes
#             subprocess.run(['docker', 'volume', 'rm', self.docker_volume], check=True)
...
#     """Memory-optimized embedding model with caching"""
...
#         memory_cache_size: int = 10000,  # Number of embeddings to keep in memory
#         cache_ttl: int = 7,  # Cache TTL in days
#         use_disk_cache: bool = True
#     ):
#         self.max_length = max_length
#         self.cache_dir = Path(cache_dir)
#         self.cache_ttl = timedelta(days=cache_ttl)
...
#         # Initialize memory cache with LRU mechanism
#         self._memory_cache = lru_cache(maxsize=memory_cache_size)(self._compute_embedding)
...
#         except Exception as e:
#             self.logger.warning(f"Failed to save cache index: {str(e)}")
...
#         cache_time = datetime.fromisoformat(self.cache_index[cache_key])
#         return datetime.now() - cache_time <= self.cache_ttl
    
#     def _save_to_disk_cache(self, text: str, embedding: np.ndarray):
...
#             self.cache_index[cache_key] = datetime.now().isoformat()
#             self._save_cache_index()
            
#         except Exception as e:
#             self.logger.warning(f"Failed to save to disk cache: {str(e)}")
...
#         except Exception as e:
#             self.logger.warning(f"Failed to load from disk cache: {str(e)}")
#             return None
...
#             for cache_key, cache_time in self.cache_index.items():
#                 if current_time - datetime.fromisoformat(cache_time) > self.cache_ttl:
...
#         # First check memory and disk cache
#         for i, text in enumerate(texts):
#             # Try memory cache first
#             embedding = self._memory_cache(text)
            
#             if embedding is None and self.use_disk_cache:
#                 # Try disk cache
#                 embedding = self._load_from_disk_cache(text)
...
#             try:
#                 # Process in batches
#                 for i in tqdm(range(0, len(texts_to_compute), batch_size), disable=not show_progress):
#                     batch = texts_to_compute[i:i + batch_size]
                    
#                     # Clear cache before each batch if using CUDA
#                     if self.device == "cuda":
#                         torch.cuda.empty_cache()
...
#                         self._memory_cache(text)  # Update memory cache
#                         if self.use_disk_cache:
#                             self._save_to_disk_cache(text, embedding)
                    
#                     embeddings.extend(batch_embeddings)
                    
#                     # Clear memory after each batch if using CUDA
#                     if self.device == "cuda":
#                         torch.cuda.empty_cache()
...
#                 cls._instance = super(CachedModelManager, cls).__new__(cls)
...
#             with self._lock:
#                 if model_name not in self._models:  # Double-check pattern
...
# class CachedEmbeddingModel:  # This is your new main embedding model class
#     """Handles model loading and embedding generation with caching"""
...
#         self.model = self.model_manager.get_model(model_name, self.device)
#         self.dimension = self.model.get_sentence_embedding_dimension()
...
#             # Try disk cache
#             if embedding is None and cache_key in self.cache_index:
#                 try:
...
#             self.client.close()
#         except Exception as e:
#             self.logger.warning(f"Error closing ChromaDB client: {str(e)}")


# chromadb working one (when cached everything; but when it's not cached dont work i think __dict__ err)
# class ChromaDBVectorStore:
#     """Handles vector storage and retrieval using ChromaDB"""
...
#             return str(list(value))  # Convert lists/tuples to string representation
#         elif isinstance(value, dict):
#             return str(value)  # Convert dictionaries to string representation
#         else:
...
#             chunk_overlap=128,
#             language="az",
#             cache_dir="embeddings_cache"  # Specify cache directory
#         )
...
#                     print("\nSummaries:")
#                     print("Extractive:", query_result['summaries']['extractive_summary'])
#                     print("Abstractive:", query_result['summaries']['abstractive_summary'])
                    
#                     print("\nLLM Response:")
#                     print(query_result['response'])
                    
#                     print(f"\nContext (repacked data): {query_result['context']}")

#                     print("\nDetailed Results:")
#                     for res in query_result['results']:
#                         print(f"\nRank: {res['rank']}")
#                         print(f"Score: {res['score']:.4f}")
#                         print(f"Matching Query: {res['matching_query']}")
...
# def main():
#     """Modified main function to process questions from Excel and write answers back"""
...
#             # Add new columns for answers if they don't exist
#             if 'extractive_answer' not in df.columns:
#                 df['extractive_answer'] = ''
#             if 'abstractive_answer' not in df.columns:
#                 df['abstractive_answer'] = ''
#             if 'llm_response' not in df.columns:
#                 df['llm_response'] = ''
                
#             # Process each question
#             for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing questions"):
#                 query = row['question']  # Assuming 'question' is the column name
...
#                     if query_result['status'] == 'success':
#                         # Store the answers in DataFrame
...
#                         df.to_excel('hr_questions_with_answers_bge_start_to_end_sep_q_fix_and_aug_new_summarizer.xlsx', index=False)
                        
#                         # Print progress
#                         print(f"\nExtracted answer: {query_result['summaries']['extractive_summary']}")
#                         print(f"Generated answer: {query_result['summaries']['abstractive_summary']}")
#                         print(f"Relevant Context info: {query_result['summaries']['relevant_info']}")
#                         print(f"LLM Response: {query_result['response']}")
                        
#                     else:
#                         print(f"Error processing query: {query_result.get('message', 'Unknown error')}")
                        
#                     # Add a small delay between questions to prevent overwhelming the system
#                     time.sleep(1)
...
# streamlit (using cache) # dont work)
# import streamlit as st
# import torch
# import logging
# from typing import List, Dict, Any
...
# class PipelineState:
#     """Global state management for pipeline"""
#     _instance = None
#     _initialized = False
#     _pipeline = None
...
# def check_milvus_collection(collection_name: str) -> bool:
...
#         return False
#     except Exception as e:
#         logger.error(f"Error checking Milvus collection: {e}")
#         return False

# @st.cache_resource(show_spinner=False)
# def initialize_rag_pipeline():
#     """Initialize RAG pipeline with proper caching"""
#     if PipelineState.is_initialized():
#         return PipelineState.get_pipeline()
...
#         # Initialize pipeline without processing document
#         pipeline = RAGPipeline(
#             collection_name="azerbaijan_docs",
#             chunk_size=1024,
#             chunk_overlap=128,
...
#         collection_exists = check_milvus_collection("azerbaijan_docs")
        
#         # Process document only if necessary
#         if not collection_exists:
#             with st.spinner("Initial setup in progress (this will take a few minutes)..."):
#                 doc_result = pipeline.process_document("formatted_combined.pdf")
#                 if doc_result['status'] != 'success':
#                     raise Exception(f"Error processing document: {doc_result.get('message', 'Unknown error')}")
                
#                 # Save metadata
#                 with open(cache_dir / "pipeline_metadata.json", "w") as f:
#                     json.dump({
#                         "initialized_at": time.strftime("%Y-%m-%d %H:%M:%S"),
#                         "document_chunks": doc_result.get('chunks', 0)
#                     }, f)
        
#         PipelineState.set_pipeline(pipeline)
#         return pipeline
...
#     """Process a query with result caching"""
#     try:
#         pipeline = PipelineState.get_pipeline()
#         if pipeline is None:
